    ...
    private void load(HttpExchange h) throws IOException {
        try {
            ...
            if ("GET".equals(h.getRequestMethod())) {
                ...
                System.out.println("KVServer --> " + key);
                if (data.containsKey(key)) {
                    System.out.println("По ключу " + key + " найдено значение:" + data.get(key));
                    sendText(h, data.get(key));
                } else {
                    System.out.println("Не найдено значение value для ключа key = " + key);
                    h.sendResponseHeaders(400, 0);
                }
